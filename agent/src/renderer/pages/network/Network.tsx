...
const Network: React.FC = () => {
  ...
  return (
    <section className='bg-background text-text h-full flex flex-col items-start'>
      ...
      <Conections name={mainAdapter.name} ip={mainAdapter.ip} mac={mainAdapter.mac.toUpperCase()} mask={mainAdapter.mask} status={mainAdapter.ip ? 'Conectado' : 'Disconectado'} typeConection={mainAdapter.type === 'wireless' ? "Wi-fi" : 'Cabeado'}  getway={mainAdapter.networkGetway} ssidConected={mainAdapter.ssidConected} />
    </section >
  )
}
